#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpMojTvEPG/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpMojTvEPG

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpMojTvEPG
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpMojTvEPG
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpMojTvEPG/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpMojTvEPG-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#          CiefpMojTvEPG INSTALLED SUCCESSFULLY         #"
